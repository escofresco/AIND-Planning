...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        self.a_levels.append(set()) # Add empty set to planning graph
        for action in self.all_actions:
            # Iterate over all possible actions
            a_node = PgNode_a(action) # Create A node base on current action
            if a_node.prenodes.issubset(self.s_levels[level]):
                for s_node in self.s_levels[level]:
                    # Update S node children with A node
                    s_node.children.add(a_node)

                    # Update A node parents with S node
                    a_node.parents.add(s_node)

                # Add action A level to planning graph
    ...
    def add_literal_level(self, level):
        ...
        """
        self.s_levels.append(set()) # Add empty set to planning graph
        for a_node in self.a_levels[level-1]:
            # Iterate over A nodes in previous level
            for s_node in a_node.effnodes:

                # Update current A node children with S node
                a_node.children.add(s_node)

                # Update current S node parents with A node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Check for mutual exclusion with first node
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Check for mutual exclusion with second node
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # Second state is the negation of the first, and both states represent same symbol
        ...
        for s1parent in node_s1.parents:
            for s2parent in node_s2.parents:
                if not s1parent.is_mutex(s2parent):
                    # Both nodes are mutually exclusive
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            # Iterate over goals in problem
            node = PgNode_s(goal, True) # Instantiate a PgNode with the current goal
            for level, nodes in enumerate(self.s_levels):
                if node in nodes:
                    level_sum += level # Add current level cost
                    break
        return level_sum
